...
std::vector<ExecutionReport> process_orders(std::vector<Order>& orders) {
    ...
    for (Order incoming_order : orders) { // Create a modifiable copy of each incoming_order
        ...
        if (incoming_order.side == 1) { // Buy order
            ...
            while (!sell_order_books[incoming_order.instrument].empty() && sell_order_books[incoming_order.instrument].top().price <= incoming_order.price && incoming_order.quantity > 0) {
                Order sell_order = sell_order_books[incoming_order.instrument].top();
                sell_order_books[incoming_order.instrument].pop();
                ...
                if (sell_order.quantity > 0) {
                    sell_order_books[incoming_order.instrument].push(sell_order);
                }
            }
            ...
            while (!buy_order_books[incoming_order.instrument].empty() && buy_order_books[incoming_order.instrument].top().price >= incoming_order.price && incoming_order.quantity > 0) {
                Order buy_order = buy_order_books[incoming_order.instrument].top();
                buy_order_books[incoming_order.instrument].pop();
                ...
                if (buy_order.quantity > 0) {
                    buy_order_books[incoming_order.instrument].push(buy_order);
                }
            }
            if (incoming_order.quantity > 0) {
...
int main() {
    std::string input_file_path = "files/inputs/orders - large2.csv"; // The path to your CSV file
    std::string output_file_path = "files/outputs/execution_rep - large2.csv"; // Path for the output file
